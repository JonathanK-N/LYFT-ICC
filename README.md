...
1. Cr�e un projet Firebase, active Authentication (Email/Password), Firestore, et (optionnel) Functions/Storage.
2. Dans Firestore, pr�vois les collections `profiles`, `rides`, `rideRequests`, `notifications`, `events`.
...
3. Saisis les r�gles de s�curit� pour limiter la lecture/�criture aux utilisateurs authentifi�s.
4. Copie les variables Firebase dans `.env` (ou dans Railway) : `VITE_FIREBASE_*`, `VITE_FIREBASE_VAPID_KEY` si tu utilises FCM.
5. Cr�e un compte Mapbox, r�cup�re un token et renseigne `VITE_MAPBOX_TOKEN`. Limite le token aux domaines de ton h�bergement.
6. Sur Railway (ou ton infra), d�finis `RAILWAY_BUILD_NODE_VERSION=22.12.0` (Vite demande Node >= 20.19).

Une fois ces �tapes en place, l'application �coutera les documents Firestore en temps r�el et mettra � jour la carte Mapbox avec les positions des conducteurs.

Pour un guide d�taill� (r�gles Firestore, g�ocodage, partages de position), consulte `docs/integration.md`.
